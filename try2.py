...
def process_dataframe(file_paths, json_data, lum=59830, is_dy=False, adjust_weight=True):
    tree = ['Z_mass', 'Z_pt', 'wt', 'n_jets', 'n_deepbjets', 'mjj', 'jdeta', 'jdphi', 'dijetpt', 'jpt_1', 'jpt_2', 'jpt_3']
...
def split_dataframe(df, output_dir, label, n_chunks=20):  # Increase the number of chunks
    total_len = len(df)
    ...
    for i in range(n_chunks):
        ...
        chunk_df.to_pickle(os.path.join(output_dir, f"new{label}_chunk{i+1}.pkl"))
...
def save_chunks(chunks, output_dir, label):
    for i, chunk in enumerate(chunks):
        chunk.to_pickle(os.path.join(output_dir, f"new{label}_chunk{i+1}.pkl"))
...
def process_and_save_chunk(data_chunk_path, mc_chunk_path, output_dir, chunk_index):
    ...
    X = combined_chunk[['Z_mass', 'Z_pt', 'wt', 'n_jets', 'n_deepbjets', 'mjj', 'jdeta', 'jdphi', 'dijetpt', 'jpt_1', 'jpt_2', 'jpt_3']]
    y = combined_chunk['label']
    weights = combined_chunk['wt']
    
    # 分割数据为训练集和测试集
    X_train, X_test, y_train, y_test, weights_train, weights_test = train_test_split(
        X, y, weights, test_size=0.9, random_state=42, stratify=y.values
    )
    ...
    scaler_filename = os.path.join(output_dir, f'newscaler_{chunk_index}.pkl')
    joblib.dump(scaler, scaler_filename)
     
    # 将训练集和测试集保存为Pandas DataFrame
    train_df = pd.DataFrame(X_train_scaled, columns=['Z_mass', 'Z_pt', 'wt', 'n_jets', 'n_deepbjets', 'mjj', 'jdeta', 'jdphi', 'dijetpt', 'jpt_1', 'jpt_2', 'jpt_3'])
    train_df['label'] = y_train.reset_index(drop=True)
    train_df['wt'] = weights_train.reset_index(drop=True)
    
    test_df = pd.DataFrame(X_test_scaled, columns=['Z_mass', 'Z_pt', 'wt', 'n_jets', 'n_deepbjets', 'mjj', 'jdeta', 'jdphi', 'dijetpt', 'jpt_1', 'jpt_2', 'jpt_3'])
    ...
    # 保存处理后的训练集和测试集
    train_df.to_pickle(os.path.join(output_dir, f'newtrain_chunk{chunk_index}.pkl'))
    test_df.to_pickle(os.path.join(output_dir, f'newtest_chunk{chunk_index}.pkl'))

    # 返回最终合并后的DataFrame
    return final_chunk

def custom_dataloader(file_paths, batch_size):
    for file_path in file_paths:
...
def main():
    ...
    output_dir_base = '/vols/cms/xw1523/masterproject/2morevariable'
    ...
    # 加载处理后的数据块
    background_paths = [os.path.join(output_dir_base, f'newbackground_chunk{i+1}.pkl') for i in range(10)]
    MC_paths = [os.path.join(output_dir_base, f'newMC_chunk{i+1}.pkl') for i in range(10)]
    real_data_paths = [os.path.join(output_dir_base, f'newReal_Data_chunk{i+1}.pkl') for i in range(10)]
    ...
    data_chunk_paths = [os.path.join(output_dir_base, f'newdata_chunk{i+1}.pkl') for i in range(10)]
    mc_chunk_paths = [os.path.join(output_dir_base, f'newMC_chunk{i+1}.pkl') for i in range(10)]
    ...
    for i, (data_path, mc_path) in enumerate(zip(data_chunk_paths, mc_chunk_paths), start=1):
        ...
        # 保存处理后的数据块
        final_chunk.to_pickle(f'/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk{i}.pkl')

    final_chunk_paths = [
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk1.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk2.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk3.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk4.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk5.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk6.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk7.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk8.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk9.pkl',
        '/vols/cms/xw1523/masterproject/2morevariable/newfinal_chunk10.pkl'
    ]

    # 使用自定义的dataloader
    batch_size = 1000  # Reduce the batch size
    for batch in custom_dataloader(final_chunk_paths, batch_size):
        # 在这里处理每个批次的数据
        print(batch.head())
